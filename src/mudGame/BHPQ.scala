...
class BHPQ [A: ClassTag](comp:(A, A)=> Boolean){
  ...
  def enqueue(obj: A): Unit ={
    ...
    var bubble = end
     while(bubble > 1 && comp(obj, heap(bubble / 2)) == true){
  ...
  def dequeue(): A = {
    ...
    var stone = 1
    var flag = true
    while(flag == true && stone * 2 < end){
      var greaterChild = if(stone * 2 + 1 < end && comp(heap(stone*2), heap(stone * 2)) == true)
      stone * 2 + 1 else stone * 2
      if(comp(heap(greaterChild), temp) == true) {
